...
class CodeWriting(Node):
    ...
    def _process_inputs(self, raw_inputs:Dict[str,str], spatial_info:Dict[str,Dict], temporal_info:Dict[str,Dict], **kwargs)->List[Any]:
        ...
        for id, info in spatial_info.items():
            # print('调试')
            # print(info['output'])
            if info['output'][0].startswith("```python") and info['output'][0].endswith("```") and self.role != 'Normal Programmer' and self.role != 'Stupid Programmer':
                output = info['output'][0].lstrip("```python\n").rstrip("\n```")
        ...
        for id, info in temporal_info.items():
            if info['output'][0].startswith("```python") and info['output'][0].endswith("```") and self.role != 'Normal Programmer' and self.role != 'Stupid Programmer':
                ...
                temporal_str += f"Agent {id} as a {info['role']} provides the following info: {info['output'][0]}\n\n"
