...
class FinalWriteCode(Node):
    ...
    def _process_inputs(self, raw_inputs:Dict[str,str], spatial_info:Dict[str,Any], temporal_info:Dict[str,Any], **kwargs)->List[Any]:
        ...
        for id, info in spatial_info.items():
            # print('输出')
            # print(info)
            input_token_count += info['output'][1]
            output_token_count+=info['output'][2]

            if info['output'][0].startswith("```python") and info['output'][0].endswith("```"):  # is python code
