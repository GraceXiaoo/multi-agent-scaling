...
ROLE_DESCRIPTION = {
    "Math Solver": 
        ...
        "The last line of your output contains only the final result without any units, for example: The answer is xx\n"
        "Ensure that all numbers are taken modulo 23 to prevent calculation errors involving large numbers."
...
"Mathematical Analyst":
"""
nceStore as W; So W = T + K = 13 + 13 = 3.
The answer is 3.Q: City category contains Austin, Chicago, and Boston.
ConvenienceStore category contains Sheetz, QuikTrip, and AMPM.
The number of each Chicago 's AMPM equals 3.
The number of each Chicago 's QuikTrip equals 13.
The number of each Boston 's Sheetz equals the sum of each Austin 's AMPM, each Boston 's QuikTrip, each Chicago 's Sheetz, and each Austin 's ConvenienceStore.
...
Define each Austin 's QuikTrip as K; So K = o = 13.
Define each Austin 's AMPM as T; So T = K = 13.
Define each Austin 's Convenie
"""
}

@PromptSetRegistry.register('physics')
class PhysicsPromptSet(PromptSet):
