...
from dataset.physics_dataset import physics_data_process,physics_get_predict
...
def parse_args():
    ...
    parser.add_argument("--dataset_json", type=str, default="/cpfs02/user/xiaojin/xiaojin/xiaojin_cpfs01/xiaojin/AgentPrune/dataset/physics/physics.jsonl")
    ...
    parser.add_argument('--domain', type=str, default="physics",help="Domain (the same as dataset name), default 'physics'")
...
async def main():
    ...
    dataset = physics_data_process(dataset)
    current_time = Time.instance().value or time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
    Time.instance().value = current_time
    print(args.dataset_json)
    if args.dataset_json =='/cpfs02/user/xiaojin/xiaojin/xiaojin_cpfs01/xiaojin/AgentPrune/dataset/physics/physics10.jsonl':
        print(1)
        result_dir = Path(f"{AgentPrune_ROOT}/result/physics_sample10")
        result_dir.mkdir(parents=True, exist_ok=True)
    else:
        result_dir = Path(f"{AgentPrune_ROOT}/result/physics")
        result_dir.mkdir(parents=True, exist_ok=True)
    ...
    for i_batch in tqdm(range(num_batches),total=num_batches,desc='processing'):
        ...
        for task, answer, log_prob, add_loss, true_answer in zip(current_batch, raw_answers, log_probs, add_losses, answers):
            ...
            #预测答案
            predict_answer = int(physics_get_predict(answer[0][0][0]))%23
            is_solved = predict_answer==int(true_answer)
